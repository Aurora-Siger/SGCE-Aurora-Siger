...
# ============================================================
#  1. DATASET — COLÔNIA AURORA SIGER
# ============================================================
...
colonia = {
    "energia": {
        "solar":  {
            ...
            "paineis_ativos": random.randint(1, 8)       # NOVO
        },
        "eolico": {
            "geracao_kw":      random.randint(10, 50),
            "velocidade_vento": random.randint(5, 20)    # NOVO
        },
        "bateria": {
            "carga_atual":    random.randint(10, 190),
            "capacidade_max": 200
        }
    },
    "consumo": {
        ...
        "logistica":    random.randint(5, 15),           # NOVO
    ...
    "clima": {
        ...
        "tempestade_areia": random.random() < 0.2        # RENOMEADO
    }
}

historico_vento   = [8, 9, 10, 11, 12, 13, 14]
historico_energia = [20, 22, 25, 27, 30, 33, 35]

historico_turno   = [1, 2, 3, 4, 5, 6]
historico_consumo = [50, 59, 68, 77, 86, 95]            # ATUALIZADO
...
def regressao_linear(x_lista, y_lista, x_novo):
    ...
    soma_x2 = sum(x_lista[i] ** 2 for i in range(n))

    b = (n * soma_xy - soma_x * soma_y) / (n * soma_x2 - soma_x ** 2)
    a = (soma_y - b * soma_x) / n

    return round(a + b * x_novo, 1)


def prever_geracao_eolica(vento_novo):
    resultado = regressao_linear(historico_vento, historico_energia, vento_novo)
...
def prever_consumo_turno(turno_novo):
    resultado = regressao_linear(historico_turno, historico_consumo, turno_novo)
